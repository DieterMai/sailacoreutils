...
class NumberAndNumberNonblankCatTest {
	...
	private static final ConfigCase CONFIG = ConfigCase.of(ConfigOptions.number, ConfigOptions.numberNonblank);

	@ParameterizedTest
	@ArgumentsSource(InputArgumentProvider.class)
    void testCatWithNumberAndNumberNonblankToResult(InputCase inputCase) throws IOException {
	...
	@ParameterizedTest
	@ArgumentsSource(InputArgumentProvider.class)
    void testCatWithNumberAndNumberNonblankToString(InputCase inputCase) throws IOException {
        String input = InputFileProvider.getTextFor(inputCase);
        String output = OutputFileProvider.getTextFor(inputCase, CONFIG);
        CatResult expected = CatResult.of(output);
