...
class NumberNonblankAndShowTabsCatTest {
	...
	private static final ConfigCase CONFIG = ConfigCase.of(ConfigOptions.numberNonblank, ConfigOptions.showTabs);

	@ParameterizedTest
	@ArgumentsSource(InputArgumentProvider.class)
    void testCatWithNumberNonblankAndShowTabsToResult(InputCase inputCase) throws IOException {
	...
	@ParameterizedTest
	@ArgumentsSource(InputArgumentProvider.class)
    void testCatWithNumberNonblankAndShowTabsToString(InputCase inputCase) throws IOException {
        String input = InputFileProvider.getTextFor(inputCase);
        String output = OutputFileProvider.getTextFor(inputCase, CONFIG);
        CatResult expected = CatResult.of(output);
