...
class ShowNonprintingCatTest {
	...
	private static final ConfigCase CONFIG = ConfigCase.of(ConfigOptions.showNonprinting);

	@ParameterizedTest
	@ArgumentsSource(InputArgumentProvider.class)
    void testCatWithShowNonprintingToResult(InputCase inputCase) throws IOException {
	...
	@ParameterizedTest
	@ArgumentsSource(InputArgumentProvider.class)
    void testCatWithShowNonprintingToString(InputCase inputCase) throws IOException {
        String input = InputFileProvider.getTextFor(inputCase);
        String output = OutputFileProvider.getTextFor(inputCase, CONFIG);
        CatResult expected = CatResult.of(output);
