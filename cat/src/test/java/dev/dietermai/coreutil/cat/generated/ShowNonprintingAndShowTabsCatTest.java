...
class ShowNonprintingAndShowTabsCatTest {
	...
	private static final ConfigCase CONFIG = ConfigCase.of(ConfigOptions.showNonprinting, ConfigOptions.showTabs);

	@ParameterizedTest
	@ArgumentsSource(InputArgumentProvider.class)
    void testCatWithShowNonprintingAndShowTabsToResult(InputCase inputCase) throws IOException {
    ...
    @ParameterizedTest
	@ArgumentsSource(InputArgumentProvider.class)
    void testCatWithShowNonprintingAndShowTabsToString(InputCase inputCase) throws IOException {
        String input = InputFileProvider.getTextFor(inputCase);
        String output = OutputFileProvider.getTextFor(inputCase, CONFIG);
