...
class NumberNonblankAndSqueezeBlankCatTest {
	...
	private static final ConfigCase CONFIG = ConfigCase.of(ConfigOptions.numberNonblank, ConfigOptions.squeezeBlank);

	@ParameterizedTest
	@ArgumentsSource(InputArgumentProvider.class)
    void testCatWithNumberNonblankAndSqueezeBlankToResult(InputCase inputCase) throws IOException {
	...
	@ParameterizedTest
	@ArgumentsSource(InputArgumentProvider.class)
    void testCatWithNumberNonblankAndSqueezeBlankToString(InputCase inputCase) throws IOException {
        String input = InputFileProvider.getTextFor(inputCase);
        String output = OutputFileProvider.getTextFor(inputCase, CONFIG);
        CatResult expected = CatResult.of(output);
